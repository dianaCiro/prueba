...
public final class FestivosColombia {
	...
	public static List<Calendar> diasFestivos(int anio) {
		...
		incluirFechaVariablePorAnio(diaFestivo); // Ascension de Jesus
		...
		incluirFechaVariablePorAnio(diaFestivo); // Sagrado Corazon
	...
	private static Calendar calcularPascua(int anio) {
		...
		if (anio >= ANIO_1900 && anio <= ANIO_2099) {
			m = 24;
			n = 5;
		} else if (anio >= ANIO_2100 && anio <= ANIO_2199) {
			m = 24;
			n = 6;
		} else if (anio >= ANIO_2200 && anio <= ANIO_2299) {
			m = 25;
			n = 0;
		}
